...
#-----拆分Description-------------------
kegg_all_pathways$Description <- sub(" - .*", "", kegg_all_pathways$Description)
...
kegg_pathways <- ggplot(kegg_df)+
                 geom_point(aes(x=RichFactor,
                 y=fct_reorder(Description,-log10(pvalue)),
                 color=-log10(pvalue),size=Count))+
  scale_color_gradient2(midpoint = 1,low = "blue",mid = "#ffcc00",high ="#dc143c" )+
  labs(x = 'Rich Factor', y = '',title=title_text)+
