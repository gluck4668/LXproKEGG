...
kegg_signal_file <- dplyr::arrange(kegg_signal,desc(RichFactor))
...
kegg_signal <- distinct(kegg_signal,Description,.keep_all = T)
...
kegg_signal_pathways <- ggplot(kegg_signal[1:nn,])+
  ...
  labs(x = 'Rich Factor', y = '',title=title_signal_text)+
