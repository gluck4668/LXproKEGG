...
# keytypes(org.Mm.eg.db) 查看org.Mm.eg.db包含的关键条目
  ...
  go_result$Description <- sub("(.*?,.*?),.*", "\\1", go_result$Description)

  names(go_result) <- gsub("Gene","Protein",names(go_result),ignore.case = T)
 ...
 for(i in 1:nn_row){
    if(nchar(as.character(go_df$Description[i]))>20)
      go_df$ProteinID[i] <- paste(str_sub(as.character(go_df$Description[i]),1,20),"...") else
        go_df$ProteinID[i] <- as.character(go_df$Description[i])
  }

  go_df <- distinct (go_df,Description,.keep_all = T)
  ...
  GO_plot <- ggplot(go_df, aes(x = Description,y =log10pvalue, fill = ONTOLOGY))
  ...
  GO_enrich01 <- GO_plot+ geom_bar(position = "dodge",stat = "identity",width = 0.8)+
    ...
    labs(x = '', y = '-log10(pvalue)',title="Protein GO enrichment")+
